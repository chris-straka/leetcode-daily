...
pub async fn scores(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let guild_data = db.get(&guild_id).ok_or("Server not configured. Run /channel first.")?;

    let mut leaderboard: Vec<_> = guild_data.users.iter().collect();
    leaderboard.sort_by(|a, b| b.1.score.cmp(&a.1.score).then_with(|| b.1.monthly_record.cmp(&a.1.monthly_record)));
    ...
    for (place, (user_id, status)) in leaderboard.into_iter().enumerate() {
        if status.score > 0 {
            has_score = true;
            msg.push_str(&format!("{}. <@{}> \t**{}** pts \t**{}** completed this month\n", 
                place + 1, user_id, status.score, status.monthly_record));
        }
    }
    if !has_score { msg.push_str("No one has any points yet."); }
    ctx.say(msg).await?;
    Ok(())
}
...
#[derive(poise::ChoiceParameter)]
pub enum PaidStatus { Free, Paid }

#[poise::command(slash_command)]
pub async fn random(ctx: Context<'_>, difficulty: Option<Difficulty>, status: Option<PaidStatus>) -> Result<(), Error> {
    let qs = crate::leetcode::fetch_all_questions().await.map_err(|e| e.to_string())?;

    let filtered: Vec<_> = qs.iter().filter(|q| {
        let diff_match = difficulty.as_ref().map_or(true, |d| q.difficulty.eq_ignore_ascii_case(&d.name()));
    ...
    if let Some(q) = question {
        let slug = q.title.to_lowercase().replace(' ', "-").chars().filter(|c| c.is_alphanumeric() || *c == '-').collect::<String>();
        let link = format!("/problems/{}", slug);
        let embed = crate::leetcode::create_embed(&q, &link);
        ctx.send(poise::CreateReply::default().content("Here's a random question!").embed(embed)).await?;
    } else {
        ctx.say("No questions found matching your criteria.").await?;
    }
    Ok(())
}
...
pub async fn channel(ctx: Context<'_>, channel: serenity::GuildChannel) -> Result<(), Error> {
    ...
    ctx.data().save().await;
    ctx.say(format!("Configured! Broadcasting to <#{}> and enrolled members.", channel.id)).await?;
    Ok(())
}
...
pub async fn poll(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let select_menu = serenity::CreateSelectMenu::new("favourite_submission", serenity::CreateSelectMenuKind::User { default_users: None });
    ...
    let sent_msg = thread_id.send_message(ctx.http(), serenity::CreateMessage::new().content(msg).components(components)).await?;
...
#[poise::command(slash_command)]
pub async fn register(ctx: Context<'_>, 
    ...
    ctx.say(format!("✅ Linked LeetCode account: **{}**", username)).await?;
    Ok(())
}
