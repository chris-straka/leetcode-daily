                ...
                if guild_data.active_daily && Some(msg.channel_id) == guild_data.thread_id {
                    ...
                    if user.submitted.is_none() {
                        ...
                        msg.reply(
                            ctx,
                            format!(
                                "✅ Verified via API! +**{}** pts. Month total: **{}**.",
                                score, user.monthly_record
                            ),
                        )
                        .await?;
                        data.save().await;
            ...
            if let serenity::Interaction::Component(component) = interaction {
                if component.data.custom_id == "favourite_submission" {
                        ...
                        &component.data.kind
                    {
                        ...
                        let mut db = data.db.write().await;
                        let user = db
                            .entry(component.guild_id.unwrap())
                            .or_default()
                            .users
                            .entry(component.user.id)
                            .or_default();
